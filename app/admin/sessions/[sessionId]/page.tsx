...
const RiskScoreItem = ({ item }: { item: { key: string; score: number | null; text: string | null } }) => (
  ...
  >
    ...
    <div style={{ marginTop: 8, color: item.text ? "var(--text)" : "var(--muted)", lineHeight: 1.5 }}>
    ...
    >
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginTop: 14, marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 26, fontWeight: 950, color: "var(--text-strong)", lineHeight: 1.15 }}>
              {aiTitle?.trim() || "—"}
            </div>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <Pill text={`Effective: ${status ?? "-"}`} tone={statusToTone(status)} />
